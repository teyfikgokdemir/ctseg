...
try {
  ...
  const cases = [
    ...
    { name:'desktop-tr-glass', path:'/glass/duzce-float-glass/', width:1440, height:1000 },
    { name:'mobile-tr-glass', path:'/glass/duzce-float-glass/', width:390, height:844 },
    ...
    ,{ name:'desktop1440-zh-home', path:'/zh/', width:1440, height:1000 }
    ,{ name:'mobile390-zh-home', path:'/zh/', width:390, height:844 }
  ...
  for (const testCase of cases) {
    ...
    const result = await page.evaluate(() => ({
      ...
      ,ux:(()=>{
        ...
        const form=document.querySelector('[data-commercial-form]');const disclosure=form?.querySelector('details');const core=['intent','tradeDirection','productFamily','name','company','emailOrPhone','message','privacy'];
        ...
        return {h1Count:document.querySelectorAll('main h1').length,h1Lines:h1Box&&h1Line?Math.round(h1Box.height/h1Line):0,h1Within:Boolean(h1Box&&h1Box.left>=-1&&h1Box.right<=innerWidth+1),headingsWithin:headings.every((box)=>box.left>=-1&&box.right<=innerWidth+1),minLightContrast:readable.length?Math.min(...readable.map((x)=>x.contrast)):99,darkSecondaryCtaContrast:selectorContrast('.hero .button-secondary,.page-hero .button-secondary,.portfolio-section .button-secondary,.process-section .button-secondary,.site-footer .button-secondary,.cta-statement .button-secondary,.duzce-page .button-secondary'),formCore:form?core.every((name)=>form.querySelector(`[name="${name}"]`)):true,detailsClosed:disclosure?!disclosure.open:true,whatsappValid:whatsapp?whatsapp.href.startsWith('https://wa.me/905456782655?text='):true,emailValid:email?email.href.startsWith('mailto:info@ctseg.com.tr?subject='):true,floatingCount:floating.length,floatingTargets:floating.every((el)=>{const b=el.getBoundingClientRect();return b.width>=44&&b.height>=44}),backInitiallyHidden:back?back.getAttribute('aria-hidden')==='true'&&back.tabIndex===-1:true,headerContract:(()=>{const header=document.querySelector('.site-header,.fa-header');if(!header)return false;const s=getComputedStyle(header);return s.position==='sticky'&&parseFloat(s.top)===0})()};
      })()
      ...
      ,desktopLocaleCodeOnly:(() => {
        ...
        const expectedCode = (document.documentElement.getAttribute('data-locale') || document.documentElement.lang.slice(0,2)).toUpperCase();
        return trigger?.textContent?.trim() === expectedCode;
      })()
      ,contactEmail:(() => {
