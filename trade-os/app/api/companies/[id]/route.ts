...
export async function DELETE(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const company = await db.company.findUnique({ where: { id }, include: { _count: { select: { cases: true, quotations: true, tasks: true, activities: true, documents: true, rfqDrafts: true } } } });
